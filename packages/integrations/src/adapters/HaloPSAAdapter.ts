...
/**
 * HaloPSA adapter.
 * API: https://haloservicedesk.com/apidoc
 * Auth: OAuth 2.0 client_credentials grant
 * Resources: users, clients, tickets, sites, agents, assets,
 *            contracts, software, suppliers, actions
 */
export class HaloPSAAdapter implements IIntegrationAdapter {
  ...
  private async getToken(cfg: IntegrationConfig): Promise<string> {
    if (cfg.credentials.accessToken) {
      const exp = cfg.credentials.expiresAt ? new Date(cfg.credentials.expiresAt as string).getTime() : 0;
      if (Date.now() < exp - 60000) return cfg.credentials.accessToken as string;
    }
    ...
    if (!res.ok) throw new Error(`HaloPSA auth: HTTP ${res.status}`);
    ...
    if (data.refresh_token) cfg.credentials.refreshToken = data.refresh_token;
    return data.access_token;
  }

  /** Fetch paginated collection from HaloPSA. */
  private async fetchAll(cfg: IntegrationConfig, path: string, top: number = 100): Promise<unknown[]> {
    const token = await this.getToken(cfg);
    const items: unknown[] = [];
    let skip = 0;
    while (true) {
      const res = await fetch(`${cfg.credentials.tenantUrl}/api${path}?$top=${top}&$skip=${skip}`, {
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      });
      if (!res.ok) throw new Error(`HaloPSA ${path}: HTTP ${res.status}`);
      const data = (await res.json()) as any;
      const list = data?.records || data?.value || data || [];
      items.push(...list);
      if (!Array.isArray(list) || list.length < top) break;
      skip += top;
    }
    return items;
  }
  ...
  async sync(cfg: IntegrationConfig): Promise<SyncResult> {
    ...
    const resources = [
      "/users", "/clients", "/tickets", "/sites",
      "/agents", "/assets", "/contracts", "/software",
      "/suppliers", "/actions",
    ];
    for (const r of resources) {
      try {
        const items = await this.fetchAll(cfg, r);
        result.recordsProcessed += items.length;
        (result as any)[r.replace("/", "")] = items;
  ...
  async disconnect(cfg: IntegrationConfig): Promise<void> {
    ...
    delete cfg.credentials.refreshToken;
  }
}
