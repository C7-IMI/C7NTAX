...
1. KEK scope: single org-level KEK vs per-tenant KEK once PLAN-003 lands (recommend org-level now, per-tenant later).
2. Key store location: local `secrets/` file vs AWS KMS once PLAN-010 lands (recommend file now with KMS as provider behind the same interface).
